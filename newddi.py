...
def get_pred_label(path):
    ...
    # lenght of X is the no of sentences
    X_test = [[word2idx[w[0]] for w in s] for s in sentence_list]

    # pad all sentences with ENDPAD "O"
    X_test = pad_sequences(maxlen=100, sequences=X_test, padding="post", value = unique_words - 1)

    # labels for training and testing
    Y_test = [[tag2idx[w[2]] for w in s] for s in sentence_list]

    # pad labels whose indexes of all sentences equivalent of "O"
    Y_test = pad_sequences(maxlen=100, sequences=Y_test, padding="post", value=tag2idx["O"])
...
if __name__ == '__main__':
    ...
    for ever_dir in path_list:
        ...
        X_test, Y_test = get_pred_label(new_path)
        test_pred, idx2tag = build_model(new_path, X_test)
